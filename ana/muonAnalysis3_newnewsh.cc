...
//   vector<AnaMuon> muEvents::ev.nMuons;
//   int nMuonEvents::ev.nMuons = 1000;
//   double nMuonEvents = ::ev.nMuons;
//   double nMuonEvents = ev.MuonHit;
   double theTHETA=50.0;   // argv[3]
   double thePHI=0.0;
//   int *nMuonEvents; 
//   nMuonEvents = new int[200000];
   vector<AnaMuon> muEvents = ev.Muon
   for (int i = 0; i<nMuonEvents; i++) {
      double ntracks = ev.MuonHit[i];
     // if(ev.MuonTk != 1) continue;
      for (int t=0; t<ev.MuonTk.siz(); t++) {
           double MuonTk = ev.MuonME[t];
           Track tk = nMuons[i].MuonTk[t];
